...
import net.tfminecraft.simplefactions.war.resolution.CouncilPeaceService;
...
import net.tfminecraft.simplefactions.government.movement.CoupService;
...
public class Faction {
	...
	public Faction(
		...
		int capital, 
		List<String> laws,
		net.tfminecraft.simplefactions.database.GovernmentData governmentData
	) {
		this.id = id;
		this.name = name;
		this.leader = leader;
		...
		if (specificTaxes != null) {
			for (Map.Entry<String, HashMap<String, Double>> entry : specificTaxes.entrySet()) {
				try {
					net.tfminecraft.simplefactions.government.proposal.TaxTarget target = net.tfminecraft.simplefactions.government.proposal.TaxTarget.valueOf(entry.getKey());
	...
	public void setLeader(String leader) {
		...
		this.leader = leader;
		net.tfminecraft.simplefactions.mercenary.contract.MercenaryLoyaltyWatcher.onGovernmentChanged(this);
	}
	...
	public void updatePrestige() {
		...
		double members = Math.pow(roster.size()+4, 1.8)+5
				+ net.tfminecraft.simplefactions.prestige.MemberPlaytime.totalFor(roster);
		...
		double tradeAmount = net.tfminecraft.simplefactions.prestige.TradePrestige.fromTradePower(
